...
The first step is to set the working directory to the Bias sub-folder by clicking on “Change dir…”:
...
First, go back to the “File conversion” tab, remove the files already loaded by clicking on the button located just below the “-” button, and by clicking on the “+” button select and load the flat frames located in the Flats subfolder.

Again, set the working directory, now to the Flats sub-folder by clicking on “Change dir…”, and set Sequence name as flats.

Like for the bias frames, “Debayer” should be left unchecked, and click on “Convert”.
...
As for the bias and flats, you need to load the dark frames. Go back to the “File conversion” tab, remove the files already loaded, select and load the dark frames located in the Darks subfolder. 

Set the working directory, now to the Darks sub-folder by clicking on “Change dir…”, and set Sequence name as darks.

Again, “Debayer” should be left unchecked, and click on “Convert”.
...
As before, select the light frames in the “File conversion” tab.

Also, set the Sequence name to lights, and point the working directory to the Lights (Change dir...). Then convert the files, still without debayering

Then go to the “Pre-Processing” tab, check “Use dark” and select the Darks/dark_stacked.fit file, then check “Use flat” and select the Flats/pp_flat_stacked.fit file. 
...
Next, go to the “Stacking” tab and make sure that “Average with rejection” is still selected as stacking method, and that “Additive with scaling” is set for Normalisation. Click on “Start stacking”. The resulting aligned and stacked image will be saved as Lights\r_db_pp_light_stacked.fit. At this step, you can also save the resulting image as JPEG, TIFF, PNG, etc. for further processing in your favorite image editor. On the menu, just click on File. Save As and pick the image format you wish (or right-click on the RGB windows and pick the format that best suits you)
