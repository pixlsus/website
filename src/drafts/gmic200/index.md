...
lede-attribution: "<a href='https://www.flickr.com/photos/philipphaegi/13987987466' title='Then we shall all burn together'>Cosmos</a> by <a href='https://www.flickr.com/photos/philipphaegi/'>Philipp Haegi</a> <a class='cc' href='https://creativecommons.org/licenses/by-nc-sa/2.0/'>cbna</a>"
...
the release of a new major version (numbered **2.0**) of its project [_G'MIC_](http://gmic.eu), a generic and extensible _open source_ framework for
[image processing](https://en.wikipedia.org/wiki/Image_processing).
Here we present the main advances made on this set of software interfaces since our [last article](https://pixls.us/blog/2016/05/g-mic-1-7-1/).
The new stuffs presented here include the work carried out over the last twelve months (versions _2.0.0_ and _1.7.x_, for _x_ varying from _2_ to _9_).
...
_G'MIC_ is an open-source project started in August 2008, in the [IMAGE](https://www.greyc.fr/en/image) team.
This French research team is specialized in the fields of algorithms and mathematics for image processing.
...
and is available for different platforms (_GNU/Linux_, _MacOS_ and _Windows_).
...
with floating-point pixel values. This includes of course "classical" color images.
...
To date, it proposes more than _480_ different filters and effects to apply to your images, and thus considerably enrich the list of image processing filters
available by default in _GIMP_.

_G'MIC_ also provides a quite powerful and autonomous [command-line interface](http://gmic.eu/reference.shtml), which is complementary
...
Other (but more confidential) _G'MIC_-based interfaces exist: a webcam streaming tool [_ZArt_](https://www.youtube.com/watch?v=k1l3RdvwHeM),
...
The proposed functionalities cover a wide spectrum of the image processing field, offering algorithms for geometrical manipulation, colorimetric changes,
...
It is therefore a very generic tool for various uses, very useful on the one hand for converting, visualizing and exploring image data,
...
the team) thus proposes a _Qt_-based version of the plug-in interface, being as independent as possible of the widget _API_ provided by _GIMP_.
...
proposed by the _GIMP_ plug-in _API_: for instance, the preview window becomes resizable at will, manages zooming by mouse wheel, and can be freely moved
...
or dark theme. The management of favorite filters has been also improved and the interface even proposes a new mode for setting the visibility of the filters.
Interface personalization is now a reality.

- The plug-in also defines its own _API_, to facilitate its integration in third party software (other than _GIMP_).
...
as comfortable as possible, have been the main objectives of this complete redesign. As you can imagine, this rewriting required a long and sustained work,
...
# 3. Ease the work of cartoonists...


But one of _G'MIC's_ purpose is also to offer more and more filters and functions to process images.
...
In particular, this version comes with new and improved filters to ease the colorization of line-arts. Indeed, we had the chance to receive the artist
...
in these fields (in particular, its webcomic [_Pepper & Carrot_](https://www.peppercarrot.com/) is a must-read!).
...
<figcaption>
<i>Fig.3.1:</i> Use of the « <b>Colorize lineart [smart coloring]</b> » filter in <i>G'MIC</i>.
</figcaption>
</figure>

When drawing comics, the colorization of line-arts is carried out in two successive steps:
...
in the drawing (_Fig.3.2.[3]_). In a second step, the colourist reworks this pre-coloring, adding shadows, lights and modifying the colorimetric ambience,
...
This algorithm smoothly manages anti-aliased contour lines, and proposes two modes of colorization:
...
to each existing region synthetized by the algorithm.
...
But, the wisest of you know that _G'MIC_ already had a [line-art colorization filter](http://www.davidrevoy.com/article240/gmic-line-art-colorization)!
...
<figcaption>
<i>Fig.3.6:</i> Filter « <b>Smooth [antialias]</b> » smoothes contours to reduce aliasing effect in cartoons (source: <i>David Revoy</i>).
</figcaption>
</figure>
...
All fantasies are then permitted, as in the example below, where the filter has been used to change the colorimetric ambience of a landscape,
mainly altering the color of the sky.
Of course, the synthetized _CLUT_ can be saved as a file and reused later for other photographs,
...
_Fig.4.3.2: Changing the color of a car, using filter « **Color mask [interactive]** » (video tutorial)._
...
_Fig.4.3.3: Changing the color of the sky in a landscape, using filter « **Color mask [interactive]** » (video tutorial)._
...
Then, for simulation [a longer exposure time](https://patdavid.net/2013/09/faking-nd-filter-for-long-exposure.html) by averaging photographs taken at
successive times. And finally, calculating averages of various kind of images for artistic purposes (for example, frames of
[music video clips](https://patdavid.net/2013/12/mean-averaged-music-videos-g.html),
[coverages of _Playboy_ magazine](https://patdavid.net/2012/08/imagemagick-average-blending-files.html) or
...
using the _CLI_ tool `gmic` provided by _G'MIC_. The example below shows the results obtained from a subsequence of the
...
And to stay in the field of video processing, we can also mention the apparition of the commands `-morph_files` and `-morph_video` that render temporal interpolations
of video sequences, taking the estimated intra-frame oject motion into account, thanks to a quite smart variational and multi-scale estimation algorithm.
...
_Fig.5.1.4: Video retiming using G'MIC temporal morphing technique._


## 5.2. Deformations and « Glitch Art »

Those who like to maltreat their images even more aggressively will be delighted to learn that a bunch of new image deformation and degradation effects
have appeared in _G'MIC_.

First of all, the filter « **Deformations / Conformal maps** » allows to distort an image by [conformal maps](https://en.wikipedia.org/wiki/Conformal_map).
...
look like image encoding or compression artefacts. First, with the effect « **Degradations / Pixel sort** » which sorts the pixels of a picture by row or by
column according to different criteria and to possibly masked regions, as this has been initially described on
...
But also with its two little brothers, filters « **Degradations / Flip & rotate blocks** » and « **Degradations / Warp by intensity** ».
...
What else do we have in store? A new image abstraction filter, named **Artistic / Sharp abstract**, based on the _Rolling Guidance_ algorithm mentioned before.
...
In the same idea, _G'MIC_ also gets a filter **Artistic / Posterize** which degrades an image to simulate [posterization](https://en.wikipedia.org/wiki/Posterization).
...
## 5.4. Other various filters

If you still want more (and in this case one could say you are damn greedy!), we propose to end this section by extirping
some of the new but unclassifiable filters out of our boxes.
...
Obviously, all filters presented throughout this article constitute only the emerged part of the _G'MIC_ iceberg.
...
---
_Credit: Unless explicitely stated, the various non-synthetic images that illustrate this post come from [_Pixabay_](https://pixabay.com/en/)._
