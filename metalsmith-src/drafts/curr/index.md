...
There can be chromatic alias problems resulting in odd color fringing and roughness on edges or a loss of detail and sharpness.
...
We're going to look at some 100% crops from that article and compare them to the results available using RawTherapee (with Pixel Shift support, obviously).
...
Thanks to the hard work of [@Ingo][] and [@Ilias][] Pentax shooters now have a Pixel Shift capable software that is not only FLOSS but also produces better results than the proprietary solutions in some cases!
